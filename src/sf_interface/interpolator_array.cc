...
void
load_interpolator_pipeline_v8( load_interpolator_pipeline_args_t * args,
                               int pipeline_rank,
                               int n_pipeline ) {
  interpolator_t * ALIGNED(128) fi = args->fi;
  const field_t  * ALIGNED(128) f  = args->f;
  ...
  const v8float fourth(0.25);
  const v8float half(  0.5 );

  const v8int   sgn_1_2(  0, 1<<31, 1<<31,     0 );
  const v8int   sgn_2_3(  0,     0, 1<<31, 1<<31 );
  const v8int   sgn_1_3(  0, 1<<31,     0, 1<<31 );
  const v8int   sel_0_1( -1,    -1,     0,     0 );
...
# define LOAD_STENCIL()    \
  ...
  for( ; n_voxel; n_voxel-- ) {
    ...
    // ex interpolation coefficients 
    w0 = toggle_bits( sgn_1_2, v8float( pf0->ex) ); // [ w0 -w0 -w0 w0 ]
    ...
    // ey interpolation coefficients 
    w0 = toggle_bits( sgn_1_2, v8float( pf0->ey) ); // [ w0 -w0 -w0 w0 ]
    ...
    // ez interpolation coefficients 
    w0 = toggle_bits( sgn_1_2, v8float( pf0->ez) ); // [ w0 -w0 -w0 w0 ]
    ...
    w0  = toggle_bits( sgn_1_3,
                       merge( sel_0_1,
                              v8float(pf0->cbx),
                              v8float(pf0->cby) ) ); // [ w0x -w0x w0y -w0y ]
    w1  =              merge( sel_0_1,
                              v8float(pfx->cbx),
