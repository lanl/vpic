...
#define DECLARE_STENCIL()                                                      \
    ...
    const int nx = g->nx, ny = g->ny, nz = g->nz;                              \
                                                                               \
    const float px_muz =                                                       \
        ( ( nx > 1 ) ? g->cvac * g->dt * g->rdx : 0 ) * m->rmuz;               \
    const float px_muy =                                                       \
        ( ( nx > 1 ) ? g->cvac * g->dt * g->rdx : 0 ) * m->rmuy;               \
    const float py_mux =                                                       \
        ( ( ny > 1 ) ? g->cvac * g->dt * g->rdy : 0 ) * m->rmux;               \
    const float py_muz =                                                       \
        ( ( ny > 1 ) ? g->cvac * g->dt * g->rdy : 0 ) * m->rmuz;               \
    const float pz_muy =                                                       \
        ( ( nz > 1 ) ? g->cvac * g->dt * g->rdz : 0 ) * m->rmuy;               \
    const float pz_mux =                                                       \
