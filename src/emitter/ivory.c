...
void
ivory( struct emitter_t * RESTRICT e,              /* Actual emitter */
       const interpolator_t * RESTRICT ALIGNED fi, /* field interp */
       field_t * RESTRICT ALIGNED f,               /* rhob accum */
       accumulator_t * RESTRICT ALIGNED a,         /* injected current accum*/
       grid_t * RESTRICT g,                        /* matching grid */
  ...
  for( n=0; n<e->n_component; n++ ) {
     ...
     /*  Notes:
        - ex, ey and ez in the interpolator for a cell are the average values of the
          fields in that cell
        - based on ivory law (proportional to a child langmuir law with a threshold)
