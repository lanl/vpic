...
void vpic_simulation::dump_energies(const char *fname,
                                    int append)
{
  double en_f[6], en_p;
  species_t *sp;
...
static FieldInfo fieldInfo[12] = {
    ...
    {"Cell Material", "SCALAR", "1", "INTEGER", sizeof(material_id)}}; // fieldInfo
  ...
  if (dumpParams.format == band)
  {
    ...
    if (istride == 1 && jstride == 1 && kstride == 1)
      ...
      for (size_t v(0); v < numvars; v++)
      {
        for (size_t k(0); k < nzout + 2; k++)
        {
          const size_t koff = (k == 0) ? 0 : (k == nzout + 1) ? grid->nz + 1
                                                              : k * kstride - 1;
          for (size_t j(0); j < nyout + 2; j++)
          {
            const size_t joff = (j == 0) ? 0 : (j == nyout + 1) ? grid->ny + 1
                                                                : j * jstride - 1;
            for (size_t i(0); i < nxout + 2; i++)
            {
              const size_t ioff = (i == 0) ? 0 : (i == nxout + 1) ? grid->nx + 1
                                                                  : i * istride - 1;
              const uint32_t *fref = reinterpret_cast<uint32_t *>(&field_array->f(ioff, joff, koff));
              fileIO.write(&fref[varlist[v]], 1);
              if (rank() == VERBOSE_rank)
                printf("%f ", field_array->f(ioff, joff, koff).ex);
              if (rank() == VERBOSE_rank)
                std::cout << "(" << ioff << " " << joff << " " << koff << ")" << std::endl;
            }
            if (rank() == VERBOSE_rank)
              std::cout << std::endl
                        << "ROW_BREAK " << joff << " " << koff << std::endl;
          }
          if (rank() == VERBOSE_rank)
            std::cout << std::endl
 ...
 */
void vpic_simulation::init_buffered_particle_dump(const char *sp_name, const int N_timesteps, const double safety_factor)
{
  species_t *sp = find_species_name(sp_name, species_list);
  if (!sp)
...
#define UNVOXEL(rank, ix, iy, iz, nx, ny, nz)             \
  BEGIN_PRIMITIVE                                         \
  {                                                       \
    int _ix, _iy, _iz;                                    \
 ...
 */
void vpic_simulation::accumulate_buffered_particle_dump(const char *sp_name, const int frame)
{
  species_t *sp = find_species_name(sp_name, species_list);
  if (!sp)
    ERROR(("Invalid species \"%s\"", sp_name));
  ...
  if (!sp->buf_size)
    ERROR(("No buffer setup on species \"%s\"", sp_name));

  if (frame < 0 || frame >= sp->buf_n_frames)
    ERROR(("Invalid frame %ld (has to be in (0;%ld( on species \"%s\"", frame, sp->buf_n_frames, sp_name));

  if (sp->np > sp->buf_n_particles)
    WARNING(("We have %d particles but buffer space for only %ld in species \"%s\". You will loose information. Chose a larger safety_factor next time.", sp->np, sp->buf_n_particles, sp_name));
 ...
 */
void vpic_simulation::write_buffered_particle_dump(const char *dname, const char *sp_name)
{
  species_t *sp = find_species_name(sp_name, species_list);
  if (!sp)
    ERROR(("Invalid species \"%s\"", sp_name));
  ...
  if (total_entries > 0)
  {
    ...
    // dX
    if (H5Lexists(subgroup, "dX", pl_link))
    {
      H5Ldelete(subgroup, "dX", pl_link);
    }
    ...
    H5Dclose(dset);
    // dY
    if (H5Lexists(subgroup, "dY", pl_link))
    {
      H5Ldelete(subgroup, "dY", pl_link);
    }
    ...
    H5Dclose(dset);
    // dZ
    if (H5Lexists(subgroup, "dZ", pl_link))
    {
      H5Ldelete(subgroup, "dZ", pl_link);
    }
    ...
    H5Dclose(dset);
    // i
    if (H5Lexists(subgroup, "i", pl_link))
    {
      H5Ldelete(subgroup, "i", pl_link);
    }
    ...
    H5Dclose(dset);
    // Ux
    if (H5Lexists(subgroup, "Ux", pl_link))
    {
      H5Ldelete(subgroup, "Ux", pl_link);
    }
    ...
    H5Dclose(dset);
    // Uy
    if (H5Lexists(subgroup, "Uy", pl_link))
    {
      H5Ldelete(subgroup, "Uy", pl_link);
    }
    ...
    H5Dclose(dset);
    // Uz
    if (H5Lexists(subgroup, "Uz", pl_link))
    {
      H5Ldelete(subgroup, "Uz", pl_link);
    }
    ...
    H5Dclose(dset);
    // w
    if (H5Lexists(subgroup, "w", pl_link))
    {
      H5Ldelete(subgroup, "w", pl_link);
    }
...
// ID
    ...
    if (sp->has_ids)
    {
      if (H5Lexists(subgroup, "q", pl_link))
      {
        H5Ldelete(subgroup, "q", pl_link);
      }
...
// dX
    ...
    if (sp->has_annotation)
    {
      for (int a = 0; a < sp->has_annotation; a++)
      {
        ...
        if (H5Lexists(subgroup, strbuf, pl_link))
        {
          H5Ldelete(subgroup, strbuf, pl_link);
        }
...
#endif
    // timestep
    if (H5Lexists(subgroup, "timestep", pl_link))
    {
      H5Ldelete(subgroup, "timestep", pl_link);
    }
  ...
  if (rank() == 0)
  {
    ...
    if (H5Lexists(subgroup, "cells", H5P_DEFAULT))
    {
    ...
    floatdata[1] = grid->dy;
    floatdata[2] = grid->dz;
    if (H5Lexists(subgroup, "resolution", H5P_DEFAULT))
    {
      H5Ldelete(subgroup, "resolution", H5P_DEFAULT);
    }
 ...
 * @return Nothing
 */
void vpic_simulation::clear_buffered_particle_dump(const char *sp_name)
{
  species_t *sp = find_species_name(sp_name, species_list);
  if (!sp)
    ERROR(("Invalid species \"%s\"", sp_name));
 ...
 * @return Nothing
 */
void vpic_simulation::interpolate_fields_annotation(const char *sp_name, const interpolator_array_t *ia,
                                                    ...
                                                    int where_bx, int where_by, int where_bz)
{
  ...
  if (!sp)
    ERROR(("Invalid species \"%s\"", sp_name));
  if (!ia)
    ERROR(("Invalid interpolator array"));
  ...
  if (where_ex < 0 && where_ey < 0 && where_ez < 0 && where_bx < 0 && where_by < 0 && where_bz < 0)
    return;
  ...
  for (p = sp->p, j = 0; j < sp->np; j++, p++)
  {
    ...
    // Store
    if (where_ex >= 0)
      sp->set_annotation(j, where_ex, ex);
    if (where_ey >= 0)
      sp->set_annotation(j, where_ey, ey);
    if (where_ez >= 0)
      sp->set_annotation(j, where_ez, ez);
    if (where_bx >= 0)
      sp->set_annotation(j, where_bx, bx);
    if (where_by >= 0)
      sp->set_annotation(j, where_by, by);
    if (where_bz >= 0)
      sp->set_annotation(j, where_bz, bz);
  }
}
 ...
 * @return Nothing
 */
void vpic_simulation::interpolate_hydro_annotation(const char *sp_name, const hydro_array_t *ha,
                                                   ...
                                                   int where_tyz, int where_tzx, int where_txy)
{
  ...
  if (where_jx < 0 && where_jy < 0 && where_jz < 0 && where_rho < 0 && where_px < 0 && where_py < 0 && where_pz < 0 && where_ke < 0 &&
      where_txx < 0 && where_tyy < 0 && where_tzz < 0 && where_tyz < 0 && where_tzx < 0 && where_txy < 0)
    return;
...
#undef VELS_RHOS
    ...
    // Store
    if (where_jx >= 0)
      sp->set_annotation(j, where_jx, jx);
    if (where_jy >= 0)
      sp->set_annotation(j, where_jy, jy);
    if (where_jz >= 0)
      sp->set_annotation(j, where_jz, jz);
    if (where_rho >= 0)
      sp->set_annotation(j, where_rho, rho);
    if (where_px >= 0)
      sp->set_annotation(j, where_px, px);
    if (where_py >= 0)
      sp->set_annotation(j, where_py, py);
    if (where_pz >= 0)
      sp->set_annotation(j, where_pz, pz);
    if (where_ke >= 0)
      sp->set_annotation(j, where_ke, ke);
    if (where_txx >= 0)
      sp->set_annotation(j, where_txx, txx);
    if (where_tyy >= 0)
      sp->set_annotation(j, where_tyy, tyy);
    if (where_tzz >= 0)
      sp->set_annotation(j, where_tzz, tzz);
    if (where_tyz >= 0)
      sp->set_annotation(j, where_tyz, tyz);
    if (where_tzx >= 0)
      sp->set_annotation(j, where_tzx, tzx);
    if (where_txy >= 0)
      sp->set_annotation(j, where_txy, txy);
  }
}
#endif
