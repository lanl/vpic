...
// Note: This is virtually identical to compute_rhob_pipeline
  ...
  for( z=2; z<=nz; z++ ) {
    for( y=2; y<=ny; y++ ) {
      ...
      fz = &f(2,y,  z-1);
      for( x=2; x<=nx; x++ ) {
        f0->div_e_err = m[f0->nmat].nonconductive *
...
void
compute_div_e_err_pipeline_v4( compute_div_e_err_pipeline_args_t * args,
                               int pipeline_rank ) {
