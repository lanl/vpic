...
reduce_array_pipeline_v4( reduce_pipeline_args_t * args,
  ...
  DISTRIBUTE( args->n, 4, pipeline_rank, n_pipeline, i, i1 ); i1 += i;
...
# define LOOP(OP) for(j=i ; j<i1; j+=4) { OP(j); }

# define A(k)   load_4x1(  &a[k],           v0   );
# define B(k,r) load_4x1(  &b[k+(r+r0)*sr], v##r );
...
reduce_array_pipeline_v4( reduce_pipeline_args_t * args,
	                        int pipeline_rank,
	                        int n_pipeline ) {
  ERROR(("No reduce_array_pipeline_v4 implementation."));
}
