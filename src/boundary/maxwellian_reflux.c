...
/*
  ...
  Refluxing boundary condition on particles.  Calculate normalized momenta
  from the various perp and para temperatures.  Then, sample Maxwellian 
  specra from Maxwellian distributsions that have the given normalized 
  momenta.  

  The perpendicular spectra are sampled from a bi-Maxwellian. 
  The parallel spectra are sampled from vpara = sqrt(2) vth sqrt(-log h)
  where h is uniformly distributed over range (0,1). 

  Strictly speaking, this method only works for non-relativistic 
  distributions of edge particles. 

  This routine requires that in the input deck one has defined arrays of 
  id[], ut_para[], ut_perp[] for each particle species.  The routine 
  creates a particle injector from the particle data passed to it.  Particle
  injection from custom boundary handlers is processed in boundary_p() in 
  boundary_p.c after particles are exchanged between processors across 
  domain boundaries.  

  Note that the way maxwellian_reflux_t is defined we have a hard maximum
  of only 32 species handled by the boundary condition.  This is not 
  dynamically sized at runtime since it makes it harder to get the 
  restart working right if we do.  If more are needed, one needs to adjust
  the size of the maxwellian_reflux_t data space by changing the array 
  sizes in boundary_handler.h  

  Procedure to generate new particle displacements:
  
  1) From face information, particle species type, and parameters for 
     species defined in the boundary handler data, obtain new Maxwellian- 
     refluxed particle momenta. 
