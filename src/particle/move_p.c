...
// accumulated to rhob and swapped with the last particle on the
// particle last (backfilling). This routine returns 0 if the particle
// could not be removed and 1 if removed successfully.
//
// Even though remove_p is internal use only, because it is seldom
// called, remove_p lightly checks its input arguments to verify
// removability.
//
// Note: for particles injected from a neighboring domain, the removal
// of the particle from the neighboring domain will accumulate the
// particle rhob at the same position on this processor where the
// opposite charged particle in accumulated (see inject_p below). When
...
// Inject a particle into the simulation. Inject first loads the
// particle position and mover according to the injector. Then it
// accumulates an equal and opposite charge to rhob (corrected local
// accumulation) to keep the charge conservation books balanced. The
// particle is then moved into its final position. Returns 0 if the
// supplied mover is no longer in use.  Returns 1 if the supplied
// mover mover is in use still in use.
//
// Arguments are lightly tested for validity. If bad arguments are
// passed, an error message is printed and 0 (mover not in use) is
// returned.
//
// Note: for particles injected from a neighboring domain, the removal
// of the particle from the neighboring domain will accumulate the
// particle rhob at the same position on this processor where the
// opposite charged particle in accumulated (see remove_p above). When
// rhob is synchronized, these charges cancel.
