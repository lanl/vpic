...
struct _particle_t;
struct _particle_mover_t;
struct _field_t;
struct _accumulator_t;
struct _grid_t;
struct _species_t;
struct _particle_injector_t;

typedef void (*boundary_handler_t)( void                     * params,
                                    struct _particle_t       * r,
                                    struct _particle_mover_t * pm,       
                                    struct _field_t          * f,
                                    struct _accumulator_t    * a,
                                    const struct _grid_t     * g,
                                    struct _species_t        * s, 
                                    struct _particle_injector_t ** ppi, 
                                    mt_handle     rng, 
                                    int           face );
...
extern void size_grid( grid_t * RESTRICT g, int lnx, int lny, int lnz );
extern void join_grid( grid_t * RESTRICT g, int bound, int rank );
extern void set_fbc( grid_t *g, int bound, int fbc );
extern void set_pbc( grid_t *g, int bound, int pbc );

/* In partition.c */
extern void partition_periodic_box( grid_t *g,
                                    double glx, double gly, double glz,
                                    int gnx, int gny, int gnz,
                                    int gpx, int gpy, int gpz );
extern void partition_absorbing_box( grid_t *g,
                                     double glx, double gly, double glz,
                                     int gnx, int gny, int gnz,
                                     int gpx, int gpy, int gpz,
                                     int pbc );
extern void partition_metal_box( grid_t *g,
                                 double glx, double gly, double glz,
                                 int gnx, int gny, int gnz,
                                 int gpx, int gpy, int gpz );
