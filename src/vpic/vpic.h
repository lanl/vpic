...
class vpic_simulation
{
  ...
  protected:
                                      ...
                                      double gpx, double gpy, double gpz )
    {
        ...
        py = size_t( gpy );
        pz = size_t( gpz );
        partition_periodic_box( grid, xl, yl, zl, xh, yh, zh, (int)gnx,
    ...
    inline material_t* define_material( const char* name, double eps,
                                        ...
                                        double zeta = 0 )
    {
        return append_material( material( name, eps, eps, eps, mu, mu, mu,
                                          sigma, sigma, sigma, zeta, zeta,
                                          zeta ),
                                &material_list );
    }
    ...
    inline material_t* define_material( const char* name, double epsx,
                                        ...
                                        double zetax = 0, double zetay = 0,
                                        double zetaz = 0 )
    {
        return append_material( material( name, epsx, epsy, epsz, mux, muy, muz,
                                          sigmax, sigmay, sigmaz, zetax, zetay,
                                          zetaz ),
                                &material_list );
    }
    ...
    inline void define_field_array( field_array_t* fa = NULL, double damp = 0 )
    {
        ...
        if ( !material_list )
            ERROR(
                ( "Define your materials before defining the field array" ) );
    ...
    inline double courant_length( double lx, double ly, double lz, double nx,
                                  double ny, double nz )
    {
        double w0, w1 = 0;
        if ( nx > 1 )
