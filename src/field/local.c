...
void local_ghost_tang_b( field_t * RESTRICT ALIGNED f,
...
void local_ghost_norm_e( field_t * RESTRICT ALIGNED f,
                         const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_ghost_div_b( field_t * RESTRICT ALIGNED f,
                        const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_tang_e( field_t * RESTRICT ALIGNED f,
                          const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
  field_t *fs;
...
void local_adjust_norm_b( field_t * RESTRICT ALIGNED f,
                          const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_div_e( field_t * RESTRICT ALIGNED f,
                         const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_jf( field_t * RESTRICT ALIGNED f,
			const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_rhof( field_t * RESTRICT ALIGNED f,
                        const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_rhob( field_t * RESTRICT ALIGNED f,
                        const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
...
void local_adjust_hydro( hydro_t * RESTRICT ALIGNED hydro,
                         const grid_t * RESTRICT g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz, nproc = mp_nproc(g->mp);
  int bc, face, x, y, z;
  hydro_t *h;
