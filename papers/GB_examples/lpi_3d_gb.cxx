...
# endif 
  ...
  species_id electron = define_species("electron", -1, max_local_np, ele_sort_int, max_local_nm); 
  ...
  species_id ion_H, ion_He; 
  if ( mobile_ions ) {
    if ( H_present  ) ion_H  = define_species("H",  Z_H /mime_H,  max_local_np, ion_sort_int, max_local_nm); 
    if ( He_present ) ion_He = define_species("He", Z_He/mime_He, max_local_np, ion_sort_int, max_local_nm); 
  }
  finalize_species(); 
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                   \
  ...
  int nspec   = 1;              // Number of electron species
  if ( mobile_ions ) {
    if ( H_present  ) nspec++; 
    if ( He_present ) nspec++; 
  }
  boundary_data->nspec=nspec; 

  int isp=0;
  boundary_data->id[isp]      = electron; 
  boundary_data->ut_perp[isp] = uth_e;     // vth_e in perp direction
  boundary_data->ut_para[isp] = uth_e;     // vth_e in para direction
  ++isp; 

  // Ions
  if ( mobile_ions ) { 
    if ( H_present ) { 
      boundary_data->id[isp]      = ion_H; 
      boundary_data->ut_perp[isp] = uthi_H; 
      boundary_data->ut_para[isp] = uthi_H; 
      ++isp; 
    } 
    if ( He_present ) { 
      boundary_data->id[isp]      = ion_He; 
      boundary_data->ut_perp[isp] = uthi_He; 
      boundary_data->ut_para[isp] = uthi_He; 
      ++isp; 
    } 
  }
...
#define ALLOCATE(A,LEN,TYPE)                                             \
  ...
  if ( step>0 && should_dump(poynting) ) {
    ...
    if ( mp_allsum_d(&psum, &gpsum, 1, grid->mp)!=NO_ERROR ) ERROR(("Bad summation in poynting diagnostic"));  
