                                      ...
                                      int ftag )
{
    ...
    sp->max_np = np_local;
#ifdef VPIC_GLOBAL_PARTICLE_ID
    size_t* sp_p_id = sp->p_id;
    size_t* p_id_buf = NULL;
    if(sp->has_ids) {
      MALLOC_ALIGNED(p_id_buf, np_local, 128);
      sp->p_id = p_id_buf;
    }
#endif
#ifdef VPIC_PARTICLE_ANNOTATION
    annotation_t* sp_p_annotation = sp->p_annotation;
    annotation_t* p_annotation_buf = NULL;
    if(sp->has_annotation) {
      MALLOC_ALIGNED(p_annotation_buf, np_local*sp->has_annotation, 128);
      sp->p_annotation = p_annotation_buf;
    }
#endif
...
#ifdef VPIC_GLOBAL_PARTICLE_ID
        if(sp->has_ids) {
          COPY(&sp->p_id[i], &sp_p_id[iptl], 1);
        }
#endif
#ifdef VPIC_PARTICLE_ANNOTATION
        if(sp->has_annotation) {
          COPY(&sp->p_annotation[i*sp->has_annotation], &sp_p_annotation[iptl*sp->has_annotation], sp->has_annotation);
        }
#endif
    }
...
#define OUTPUT_CONVERT_GLOBAL_CELL_ID 1
#ifdef OUTPUT_CONVERT_GLOBAL_CELL_ID
...
#ifdef VPIC_PARTICLE_ANNOTATION
    if(sp->has_annotation) {
      hsize_t annspace_count_temp = numparticles * sp->has_annotation;
      hid_t annspace = H5Screate_simple(1, &annspace_count_temp, NULL);
      hsize_t annspace_start = 0, annspace_stride = sp->has_annotation, annspace_count = np_local;
      H5Sselect_hyperslab(annspace, H5S_SELECT_SET, &annspace_start, &annspace_stride, &annspace_count, NULL);

      // particle annotations
      for(int j=0; j<sp->has_annotation; j++) {
        char ann_scratch[128];
        sprintf(ann_scratch, "annotation_%d", j);
#if VPIC_PARTICLE_ANNOTATION == float
        dset_id = H5Dcreate(group_id, ann_scratch, H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        ierr = H5Dwrite(dset_id, H5T_NATIVE_FLOAT, annspace, filespace, plist_id, sp->p_annotation + j);
#elif VPIC_PARTICLE_ANNOTATION == double
        dset_id = H5Dcreate(group_id, ann_scratch, H5T_NATIVE_DOUBLE, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        ierr = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, annspace, filespace, plist_id, sp->p_annotation + j);
#else
# error "Unsupported annotation type"
#endif
        H5Dclose(dset_id);
      }
    }
    ...
    if(sp->has_ids) {
      sp->p_id = sp_p_id;
      FREE_ALIGNED(p_id_buf);
    }
#endif
#ifdef VPIC_GLOBAL_PARTICLE_ANNOTATION
    if(sp->has_annotation) {
      sp->p_annotation = sp_p_annotation;
      FREE_ALIGNED(p_annotation_buf);
    }
#endif
...
vpic_simulation::write_buffered_particle_dump(const char * dname, const char * sp_name) {
  ...
  // Open file
  dump_mkdir(dname);
  sprintf(strbuf, "%s/T.%ld", dname, step()+1-sp->buf_n_frames); //Updated
  dump_mkdir(strbuf);
  sprintf(strbuf, "%s/T.%ld/tracers.h5p", dname, step()+1-sp->buf_n_frames); //Updated
  ...
  // Timestep group
  hid_t group;
  sprintf(strbuf, "Step#%ld", step()+1-sp->buf_n_frames); //Updated
  mp_barrier();
  ...
  sprintf(strbuf, "%s", "grid_metadata");
  mp_barrier();
  state = H5Lexists(group, strbuf, pl_link);
  mp_barrier();
  if(state > 0) {
  ...
  if(rank() == 0 ) {
    ...
    subgroup = H5Gopen(group, "grid_metadata", H5P_DEFAULT);
