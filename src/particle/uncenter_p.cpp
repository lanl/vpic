...
/* uncenter_p half pushes particle momentum backwards in time to uncenter it
   All particles must be inbounds */

static void uncenter_p_no_v4( particle_t * RESTRICT ALIGNED p,
                              int n,
                              const float q_m,
...
static void uncenter_p_v4( particle_t * RESTRICT ALIGNED p,
                           const int n,
                           const float q_m,
                           const interpolator_t * RESTRICT ALIGNED f0,
                           const grid_t * RESTRICT g ) {
  ...
  uncenter_p_no_v4( p, n&3, q_m, f0, g );
}

#endif

void uncenter_p( particle_t * RESTRICT ALIGNED p,
                 const int n,
                 const float q_m, 
...
#ifdef V4VERSION
  uncenter_p_v4( p, n, q_m, f, g );
#else
  uncenter_p_no_v4( p, n, q_m, f, g );
#endif
}
