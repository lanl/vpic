...
//??????????????????????????????????????????????????????????????????????????????????????????
  ...
  sim_log("- Creating electron species."); electron = define_species("electron", -1,           2.0*max_local_np, -1, ele_sort_freq, 1); 
  sim_log("- Creating I1 ion species.");   ion_I1   = define_species("I1",       Z_I1/mime_I1, 2.0*max_local_np, -1, ion_sort_freq, 1); 
  sim_log("- Creating I2 ion species.");   ion_I2   = define_species("I2",       Z_I2/mime_I2, 2.0*max_local_np, -1, ion_sort_freq, 1); 
      ...
      repeat( Ne/(topology_x*topology_y*topology_z) ) {
        ...
        if ( uniform_rand(0,1) < an1*tanh(chi*xi) + bn1 ) { 
          ...
          repeat( Z_I1 ) { 
            ...
            do {
              vx = maxwellian_rand(uthe * uthe_relativistic_correction);
              vy = maxwellian_rand(uthe * uthe_relativistic_correction);
              vz = maxwellian_rand(uthe * uthe_relativistic_correction);
              u2 = vx*vx + vy*vy + vz*vz;
            } while ( uniform_rand(0,1) < u2*u2*one_over_eight_theta );
  ...
  if ( global->do_collisions && global->load_particles && step>0 && step%global->tstep_coll==0 ) {
    ...
    for ( isp1=0; isp1<nsp; ++isp1 ) {
      ...
      for ( icell=((grid->nx+2)*(grid->ny+2)*(grid->nz+2))-2; icell>=0; --icell ) {
        ...
        for ( i=0; i<npar1; ++i ) {
          int j;
          j = uniform_rand( 0, (double)i+1 );                  // j is a random integer satisfying 0 <= j <= i
