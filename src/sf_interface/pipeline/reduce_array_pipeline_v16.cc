...
reduce_array_pipeline_v16( reduce_pipeline_args_t * args,
  ...
  DISTRIBUTE( args->n, 16, pipeline_rank, n_pipeline, i, i1 ); i1 += i;
...
# define LOOP(OP) for(j=i ; j<i1; j+=16) { OP(j); }

# define A(k)   load_16x1(  &a[k],           v0   );
# define B(k,r) load_16x1(  &b[k+(r+r0)*sr], v##r );
# define C(k,v) store_16x1( v, &a[k] )
...
reduce_array_pipeline_v16( reduce_pipeline_args_t * args,
	                        int pipeline_rank,
	                        int n_pipeline ) {
  ERROR(("No reduce_array_pipeline_v16 implementation."));
}
