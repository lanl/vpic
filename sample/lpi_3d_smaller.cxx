//========================================================================
//
// LPI 3D deck - Linearly polarized (in y) Gaussian laser beam incident 
//               from left boundary.  Deck is a smaller-core-count version
//               of the GB deck and is intended for the "physics" runs on
//               redtail.  Deck is designed to work with VPIC revision 
//               revision XXX 
//
...
//
// mkdir rundata field hydro restart poynting
//========================================================================
...
begin_globals {
  ...
  int poynting_interval;       // how often to compute poynting flux on bound.
  ...
  int rtoggle;                 // enable save of last 2 restart files 
  int load_particles;          // were particles loaded? 
  double topology_x;           // domain topology needed to normalize 
                               // Poynting diagnostic 
  double topology_y;
  double topology_z;
  int mobile_ions;
  int H_present; 
  int He_present; 
  int write_poynting_data;     // Write poynting data to file? (or stdout)
  ...
  double mask;                 // # gaussian widths from beam center where 
                               // I is nonzero
};
...
begin_initialization {
  ...
  double cfl_req    = 0.98;                // How close to Courant to run
  double damp       = 0;                   // How much radiation damping
  double iv_thick   = 2;                   // Thickness (cells) of i.v. layer
  ...
  double laser_intensity   = 1e17 * 1e7;   // in ergs/cm^2 (1 W = 1e7 ergs)
  ...
  double ny                = 258;
  double nz                = 258; 
  double topology_x        = 20;
  double topology_y        = 6;
  double topology_z        = 6;           // 48 proc

# if 0 // Reduced variant for running tests on desktop
  Lx /= 5.; nx /= 5.; topology_x /= 5.;
  Ly /= 6.; ny /= 6.; topology_y /= 6.;
  Lz /= 6.; nz /= 6.; topology_z /= 6.;
# endif

  // For GB:  Max performance = increase nppc until problem doesn't fit 
  // in memory anymore! 

  double nppc              = 25;           // Ave. # particles/cell/species
  int load_particles       = 1;            // Turn on/off particle load 
  int mobile_ions          = 0;            // Push ions?
  int He_present=0, H_present=1;           // Initialize parameters
  double f_He              = 0.0;          // Ratio of number density of He 
                                           // to total ion density
  double f_H               = 1-f_He;       // Ratio of number density of H  
                                           // to total ion density
  ...
  double n_e   = c_vac*c_vac*m_e/(4*M_PI*ec*ec*delta*delta); // e- dens (cm^-3)
  ...
  // Convert box size to skin depths
  Lx /= delta;                                    
  Ly /= delta;   
  Lz /= delta;   
  ...
  double lambda            = vacuum_wavelength/delta; // vac. wavelength (c/wpe)
  double waist             = f_number*lambda;     // beam width (c/wpe) at focus
  double xfocus            = Lx/2;                // in c/wpe
  double ycenter           = 0;                   // center of spot in y on 
                                                  // lhs boundary
  double zcenter           = 0;                   // center of spot in z on 
                                                  // lhs boundary
  double mask              = 1.5;                 // set drive I=0 outside 
                                                  // r>mask*width at lhs bound.
  double width = waist*sqrt( 1 + (lambda*xfocus/(M_PI*waist*waist))
                                 * (lambda*xfocus/(M_PI*waist*waist)) ); 
  e0                       = e0*(waist/width);    // at entrance (3D Gaussian) 
 
  double dt                = cfl_req*courant_length(Lx,Ly,Lz,nx,ny,nz); 
                                                  // in 1/wpe; 
                                                  // n.b. c=1 in nat. units

  double nsteps_cycle      = trunc_granular(2*M_PI/(dt*omega),1)+1; 
  dt                       = 2*M_PI/omega/nsteps_cycle; // nsteps_cycle time 
                                                  //   steps in one laser cycle

  double t_stop            = 1001;                // Runtime in 1/wpe
  int particle_interval    = 0; 
  int poynting_interval    = 16;                 

  int field_interval       = int(5.0/dt);         
  int restart_interval     = 1024;                
  int quota_check_interval = 20;      
  double quota_sec         = 12*3600;             // Run quota in sec. 
  int write_poynting_data  = 0;                   // Write to file or stdout?

  double N_e               = nppc*nx*ny*nz;       // # macro electrons in box
  double Np_e              = Lx*Ly*Lz;            // # "physical" e- in box 
                                                  // (nat. units)
  double q_e               = -Np_e/N_e;           // Charge per macro electron
  double N_i               = N_e;                 // # macro ions of ea. 
                                                  // species in box 
  double Np_i              = Np_e/(Z_H*f_H+Z_He*f_He); // # "physical" ions of 
                                                  // each sp. in box
...
# define iv_region (   x<      hx*iv_thick || x>Lx  -hx*iv_thick  \
  ...
  if ( load_particles ) {
    ...
    // Fast load of particles--don't fix artificial domain correlations
    ...
    repeat( N_e/(topology_x*topology_y*topology_z) ) {
      ...
      if ( iv_region ) continue;           // Don't load particle in iv_region
...
# define should_dump(x) \
 ...
 if ( step==0 ) {
  ...
  // TODO: It is assumed that Ponyting dumps are infrequent, so we can 
  // afford the mpi_allreduce().  This needs to be verified on RR hardware. 
...
# define MASK  ( R2<=pow(global->mask*global->width,2) ? 1 : 0 )
  ...
  if ( grid->x0==0 ) {               // Node is on left boundary
    ...
    double h                  = global->xfocus/rl;   // dist./Rayleigh length
