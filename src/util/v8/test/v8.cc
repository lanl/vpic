...
TEST(v8, test_splat) {
  v8int a( 1, 2, 3, 4), b( 5, 6, 7, 8), c( 9,10,11,12), d(13,14,15,16);
  v8int e(17,18,19,20);
  b = splat<0>(a); c = splat<1>(a); d = splat<2>(a); e = splat<3>(a);
  ASSERT_FALSE( any(a!=v8int(1,2,3,4)) || any(b!=v8int(1,1,1,1)) ||
      any(c!=v8int(2,2,2,2)) || any(d!=v8int(3,3,3,3)) ||
      any(e!=v8int(4,4,4,4)) );
} // TEST

TEST(v8, test_shuffle) {
  v8int a( 0, 1, 2, 3), b( 4, 8,12,16), c( 5, 9,13,17), d( 6,10,14,18);
  v8int e( 7,11,15,19);
  b = shuffle<1,2,3,0>(a);
  c = shuffle<2,3,0,1>(a);
  d = shuffle<3,0,1,2>(a);
  e = shuffle<3,2,1,0>(a);
  ASSERT_FALSE( any(a!=v8int(0,1,2,3)) || any(b!=v8int(1,2,3,0)) ||
      any(c!=v8int(2,3,0,1)) || any(d!=v8int(3,0,1,2)) ||
      any(e!=v8int(3,2,1,0)) );
} // TEST

TEST(v8, test_swap) {
  v8int a(1,2,3,4), b(5,6,7,8);
  swap(a,b);
  ASSERT_FALSE( any(a!=v8int(5,6,7,8)) || any(b!=v8int(1,2,3,4)) );
} // TEST

TEST(v8, test_transpose) {
  v8int a0( 0, 1, 2, 3), a1( 4, 5, 6, 7), a2( 8, 9,10,11), a3(12,13,14,15);
  transpose(a0,a1,a2,a3);
  ASSERT_FALSE( any(a0!=v8int( 0, 4, 8,12)) || any(a1!=v8int( 1, 5, 9,13)) ||
      any(a2!=v8int( 2, 6,10,14)) || any(a3!=v8int( 3, 7,11,15)) );
} // TEST

TEST(v8, test_load_4x1) {
  DECLARE_ALIGNED_ARRAY( int, 16, mem, 16 );
  v8int a0(1,0,0,0), a1(0,0,0,0), a2(0,0,0,0), a3(0,0,0,0);
  int i;
  ...
  ASSERT_FALSE( any(a0!=v8int( 0, 1, 2, 3)) || any(a1!=v8int( 4, 5, 6, 7)) ||
      any(a2!=v8int( 8, 9,10,11)) || any(a3!=v8int(12,13,14,15)) || i!=16 );
} // TEST

TEST(v8, test_store_4x1) {
  DECLARE_ALIGNED_ARRAY( int, 16, mem, 16 );
  v8int a0( 0, 1, 2, 3), a1( 4, 5, 6, 7), a2( 8, 9,10,11), a3(12,13,14,15);
  int i;
...
TEST(v8, test_load_4x2_tr) {
  ...
  for( i=0; i<16; i++ ) if( mem[i]!=i ) break;
  ASSERT_FALSE( any(a0!=v8int( 0, 4, 8,12)) || any(a1!=v8int( 1, 5, 9,13)) ||
      any(a2!=v8int( 2, 6,10,14)) || any(a3!=v8int( 3, 7,11,15)) || i!=16 );
} // TEST

TEST(v8, test_load_4x3_tr) {
  DECLARE_ALIGNED_ARRAY( int, 16, mem, 16 );
  v8int a0, a1, a2;
  int i;
  for( i=0; i<16; i++ ) mem[i] = i;
  ...
  for( i=0; i<16; i++ ) if( mem[i]!=i ) break;
  ASSERT_FALSE( any(a0!=v8int( 0, 4, 8,12)) || any(a1!=v8int( 1, 5, 9,13)) ||
      any(a2!=v8int( 2, 6,10,14)) || i!=16 );
} // TEST

TEST(v8, test_load_4x4_tr) {
  DECLARE_ALIGNED_ARRAY( int, 16, mem, 16 );
  v8int a0, a1, a2, a3;
  int i;
  for( i=0; i<16; i++ ) mem[i] = i;
...
TEST(v8, test_store_4x1_tr) {
  DECLARE_ALIGNED_ARRAY( int, 16, mem, 16 );
  v8int a0( 0, 4, 8,12), a1( 1, 5, 9,13), a2( 2, 6,10,14), a3( 3, 7,11,15);
  int i;
  for( i=0; i<16; i++ ) mem[i] = 0;
