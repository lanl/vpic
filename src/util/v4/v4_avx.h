...
class v4int : public v4
{
  ...
  public:
    ...
    v4int( int i0, int i1, int i2, int i3 )
    { // Init from scalars
        union {
            int i;
            float f;
...
#undef CMATH_FR2
  ...
  public:
    ...
    v4float( float f0, float f1, float f2, float f3 )
    { // Init from scalars
...
inline v4int operator&&( const v4float& a, const v4float& b )
{
    ...
    c.v =
        _mm_and_ps( _mm_cmpneq_ps( a.v, vzero ), _mm_cmpneq_ps( b.v, vzero ) );
    return c;
}
...
inline v4float rsqrt( const v4float& a )
{
    ...
    b.v = _mm_add_ps(
        ...
        _mm_mul_ps(
            ...
            _mm_sub_ps(
                b_v, _mm_mul_ps(
                         a_v, _mm_mul_ps( b_v, _mm_mul_ps( b_v, b_v ) ) ) ) ) );
    return b;
}
...
inline v4float rcp( const v4float& a )
{
    ...
    b.v = _mm_sub_ps( _mm_add_ps( b_v, b_v ),
                      _mm_mul_ps( a_v, _mm_mul_ps( b_v, b_v ) ) );
    return b;
}
...
inline void trilinear( v4float& wl, v4float& wh )
{
    ...
    __m128 z = wl.v, xy;
    xy = _mm_add_ps(
        l, _mm_xor_ps( s, _mm_shuffle_ps( z, z, PERM( 0, 0, 1, 1 ) ) ) );
    z = _mm_add_ps(
        l, _mm_xor_ps( s, _mm_shuffle_ps( z, z, PERM( 2, 2, 2, 2 ) ) ) );
