...
#define DECL_SFMT                                                              \
    __m128i a_u, mask = _mm_setr_epi32( SFMT_MASK0, SFMT_MASK1, SFMT_MASK2,    \
                                        SFMT_MASK3 )
...
#define SFMT( a, b, c, d )                                                     \
    ...
    y3 = ( c.u3 >> SFMT_R2A );                                                 \
    a.u0 ^= ( x0 ^ ( ( b.u0 >> SFMT_R1 ) & SFMT_MASK0 ) ) ^                    \
            ( y0 ^ ( d.u0 << SFMT_L1 ) );                                      \
    a.u1 ^= ( x1 ^ ( ( b.u1 >> SFMT_R1 ) & SFMT_MASK1 ) ) ^                    \
            ( y1 ^ ( d.u1 << SFMT_L1 ) );                                      \
    a.u2 ^= ( x2 ^ ( ( b.u2 >> SFMT_R1 ) & SFMT_MASK2 ) ) ^                    \
            ( y2 ^ ( d.u2 << SFMT_L1 ) );                                      \
    a.u3 ^= ( x3 ^ ( ( b.u3 >> SFMT_R1 ) & SFMT_MASK3 ) ) ^                    \
            ( y3 ^ ( d.u3 << SFMT_L1 ) )
...
#define RNG_NEXT( a, t, r, p, rs )                                             \
    ...
    {                                                                          \
        uint32_t _n =                                                          \
            ( ( r )->n + ( (uint32_t)sizeof( ( r )->state.p[0] ) ) - 1 ) &     \
            /**/ ( ~( ( (uint32_t)sizeof( ( r )->state.p[0] ) ) - 1 ) );       \
        if ( _n >= SFMT_NC )                                                   \
            sfmt_next( ( r )->state.sfmt ), _n = 0;                            \
        ( a ) =                                                                \
            ( ( r )->state.p[_n / (uint32_t)sizeof( ( r )->state.p[0] )] >>    \
              ( rs ) );                                                        \
...
#define conv_frand_c( u32 )                                                    \
    ( ( ( ( u32 ) >> 8 ) + ( (u32)&1 ) ) * ( 1.f / 16777216.f ) )

#define conv_drand( u64 )                                                      \
    ( ( ( ( u64 ) >> 12 ) + 0.5 ) * ( 2. / 9007199254740992. ) )
#define conv_drand_c0( u64 ) ( ( ( u64 ) >> 11 ) * ( 1. / 9007199254740992. ) )
#define conv_drand_c1( u64 )                                                   \
    ( ( ( ( u64 ) >> 11 ) + 1 ) * ( 1. / 9007199254740992. ) )
