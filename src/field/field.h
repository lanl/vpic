...
/* FIXME: (nx>1) ? (1/dx) : 0 TYPE LOGIC SHOULD BE FIXED SO THAT
   NX REFERS TO THE GLOBAL NUMBER OF CELLS IN THE X-DIRECTION (NOT
...
void
delete_field( field_t ** ALIGNED f );

void
clear_field( field_t * ALIGNED f,
             const grid_t * g );

void clear_jf( field_t * ALIGNED f,
               const grid_t * g );

void clear_rhof( field_t * ALIGNED f,
                 const grid_t * g );
...
interpolator_t * ALIGNED
new_interpolator( const grid_t * g );

void
delete_interpolator( interpolator_t ** ALIGNED fi );

void
clear_interpolator( interpolator_t * ALIGNED fi,
                    const grid_t * g );
...
/* Going into reduce_accumulators, the host processor and the pipeline
   processors have each accumulated values to their personal accumulators.
   This reduces the pipeline accumulators into the host accumulator with
   a pipelined horizontal reduction (a deterministic reduction). */
...
/* advance_e applies the following difference equations to the fields
   ...
   Note: advance_e is structurally the same as compute_curl_b.  Updates to
   one likely should be replicated in the other. */
...
/* This computes 6 components of field energy of the system.  The
  ...
  All nodes get the same result.  FIXME: Should this function perform the
  global reduce? */

void
energy_f( double * energy,                          /* 6 element array */
          const field_t * ALIGNED f,
          const material_coefficient_t * ALIGNED m,
          const grid_t * g );
...
/* compute_curl_b applies the following difference equations to the fields
   ...
   Note: compute_curl_b is structurally the same as advance_e; updates to
   one likely should be replicated in the other. */
...
/* compute_rhob applies the following difference equation
   ...
   this routine is identical to compute_div_e_err, so updates to one should 
...
/* compute_div_e_err applies the following difference equation:
   ...
   updates to one should likely be replicated in the other. */
...
/* compute_rms_div_e_err returns
   ...
   processor gets the same value.  Note that this function does _not_ update
   or recompute div_e_err.  FIXME: Should this function do the global
   reduce? */
...
/* compute_rms_div_b_err returns
   ...
   of all domains.  Every processor gets the same value.  Uses the value
   of div_b_err already stored.  It _does_ _not_ recompute div_b_err.
...
double
synchronize_tang_e_norm_b( field_t * ALIGNED f,
                           const grid_t * g );

void
synchronize_jf( field_t * ALIGNED f,
                const grid_t * g );

void
synchronize_rhof( field_t * ALIGNED f,
                  const grid_t * g );

void
synchronize_rhob( field_t * ALIGNED f,
                  const grid_t * g );

void
synchronize_hydro( hydro_t * ALIGNED hydro,
                   const grid_t * g );
...
/* Given a block of voxels to be processed, determine the number of voxels
   and the first voxel a particular job assigned to a pipeline should process.
   The return voxel is the number of voxels to process.
   ...
   complete bundles of 4 voxels to facillitate vector processing.  If job is
   set to n_job, this function will compute the voxel index of the first
   voxel in the final incomplete bundle and return the number of voxels
   in the final incomplete bundle. */
...
void
local_ghost_tang_b( field_t * ALIGNED f,
                    const grid_t * g );

void
local_ghost_norm_e( field_t * ALIGNED f,
                    const grid_t * g );

void
local_ghost_div_b( field_t * ALIGNED f,
                   const grid_t * g );

void
local_adjust_tang_e( field_t * ALIGNED f,
                     const grid_t * g );

void
local_adjust_div_e( field_t * ALIGNED f,
                    const grid_t * g );

void
local_adjust_norm_b( field_t * ALIGNED f,
                     const grid_t * g );

void
local_adjust_jf( field_t * ALIGNED f,
                 const grid_t * g );

void
local_adjust_rhof( field_t * ALIGNED f,
                   const grid_t * g );

void
local_adjust_rhob( field_t * ALIGNED f,
                   const grid_t * g );

void
local_adjust_hydro( hydro_t * ALIGNED h,
                    const grid_t * g );

/* In remote.c */

void
begin_remote_ghost_tang_b( field_t * ALIGNED f,
                           const grid_t * g );

void
end_remote_ghost_tang_b( field_t * ALIGNED f,
                         const grid_t * g );

void
begin_remote_ghost_norm_e( field_t * ALIGNED f,
                           const grid_t * g );

void
end_remote_ghost_norm_e( field_t * ALIGNED f,
                         const grid_t * g );

void
begin_remote_ghost_div_b( field_t * ALIGNED f,
                          const grid_t * g );

void
end_remote_ghost_div_b( field_t * ALIGNED f,
                         const grid_t * g );
