...
\title{0.383 Pflop/s and Trillion-Particle Kinetic Modeling of Laser Plasma Interaction on Roadrunner}
\author{%
...
\begin{figure}
    \begin{center}
    \scalebox{0.45}{\includegraphics{lpi}}
...
$3.0$~Pflop/s in single precision\footnote{This includes the
PPE's capability.  The Opteron base system has 
a theoretical peak performance of 93.3 Tflop/s that is not included 
...
is a 128-bit vector engine with a 256 KiB user-controlled, embedded
...
KiB are needed to hold a block---the largest size of a single DMA
transfer).  While a block is processed, the next is loaded and the
previous is stored.
%$48$ KiB of local store and $3$ channels are used
...
%table) and thus the cache requires of $134$ KiB local store overall.
...
%there $3$ such blocks, $24$ KiB of local store and $3$ DMA channels are
%used for managing unprocessed exceptions.

Overall, $206$ KiB of local store data is used for the particle advance
(leaving $50$ KiB for code) and all $32$ DMA channels are utilized.
...
200 KiB in size; the number of particles migrating to each neighbor may 
...
size of 214~KiB; from the system standpoint, the 0-byte latency from Cell 
...
\hline
Avg. Itr. Time (Hybrid) & 0.417 sec & 0.422 sec & 0.422 sec & 0.422 sec \\
Av. Itr. Time (Opteron) & 4.85 sec & 4.86 sec & 4.86 sec & 4.86 sec \\
Processing Rate (Hybrid) & 21.3 TF & 127.8 TF & 255.5 TF & \textbf{383.3 TF} \\
Processing Rate (Opteron) & 1.82 TF & 10.9 TF & 21.9 TF & 32.8 TF \\
Performance Advantage & 11.7$x$ & 11.7$x$ & 11.7$x$ & 11.7$x$ \\
\hline
\end{tabular}
\end{center}
...
\hline
 System Size & 1 CU & 6 CU & 12 CU & 18 CU \\
             & (180 Triblades) & (1080 Triblades) & (2160 Triblades) & (3240 Triblades) \\
\hline
Avg. Itr. Time (Hybrid) & 0.784 sec & 0.789 sec & 0.789 sec & 0.789 sec \\
...
This could make 400+ Tflop/s simulations of the LPI problems presented
