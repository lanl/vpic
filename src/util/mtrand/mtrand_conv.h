...
// The below is tricky. In drand53_c, multiplying by the inverse
// causes a 0.5*eps error that prevents 1 from ever being
// attained. drand53_c uses a division instead, which is unfortunately
// slower. The multiply has an error because 1/(2^53-1) does not have
// an exact representation (e.r.) in floating point but 2^53-1
// does. In the c0 and c1 cases, 1/2^53 does have an e.r. so
// multiplying is preferred for speed. drand53_o is even
// trickier. Ideally you should take a 53-bit rand, add 0.5 and
...
// higher precision (i.e. the x86 FPU) but this is not
// assumed. Similar issues apply for the frand24 and drand32 macros.
...
// precision, these macros are much less tricky. Still need a division
// in drand32_c.
